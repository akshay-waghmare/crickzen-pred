"""
NTB T20 Blast v1 Phase-Split Model Build
==========================================

Following IPL v17 pattern: PP/MID/DEATH phase-split model for innings-2.

Architecture:
  - Innings 1: Uses base model from models/ntb_v1/champion_model.joblib
  - Innings 2 Powerplay (overs 1-6): champion_model_pp.joblib
  - Innings 2 Middle (overs 7-15): champion_model_mid.joblib
  - Innings 2 Death (overs 16-20): champion_model_death.joblib

Data: data/ntb_features_v1/training.parquet (1,489 matches, 2014-2026)
"""
...
DATA_PATH = Path("data/ntb_features_v1/training.parquet")
OUT_DIR = Path("models/ntb_v1_phase")
BASE_MODEL_DIR = Path("models/ntb_v1")
...
def main() -> None:
    ...
    if not DATA_PATH.exists():
        ...
        print("Run 'bbl-pipeline retrain --league ntb --version v1' first to generate features.")
        return

    print("=" * 70)
    print("NTB T20 Blast v1 Phase-Split Model Build")
    print("=" * 70)
    ...
    for phase, feats in PHASE_FEATURES.items():
        ...
        if missing:
            print(f"  WARNING: {len(missing)} {phase} features not in data: {missing[:5]}...")
    ...
    print(
        f"  Feature counts: "
        f"PP {len(phase_feats['pp'])}, "
        f"MID {len(phase_feats['mid'])}, "
        f"DEATH {len(phase_feats['death'])}"
    )
    ...
    for phase, over_range in PHASE_RANGES.items():
        ...
        oof_rows.append({
            "phase": phase,
            "n_rows": len(pf),
            "n_features": len(phase_feats[phase]),
            "oof_brier_raw": round(oof["brier"], 5),
            "oof_brier_cal": round(oof_brier_cal, 5),
        })
        print(
            f"  {phase.upper():<5} rows={len(pf):>6,} feats={len(phase_feats[phase]):>2} "
    ...
    routing_config = {
        ...
        "description": (
            "ntb_v1_phase: T20 Blast phase-split model (PP/MID/DEATH) "
            "with XGBLRBlend + per-over calibration. "
            "Inn1 uses base model from models/ntb_v1."
        ),
        "inn1_model_dir": "models/ntb_v1",
        "inn2_phase_model_dir": str(OUT_DIR).replace("\\", "/"),
        "apply_calibration": True,
        "post_model_calibration": {
            "enabled": False,
            "reason": "Per-over calibration in phase_oof_calibrators.pkl.",
        },
        "calibration": {
            "pp": "per_over_isotonic",
            "mid": "per_over_platt",
    ...
    rows = []
    print("\n" + "=" * 80)
    print("NTB v1 Phase-Split - OOS COMPARISON  (train: pre-2025, test: 2025+2026)")
    print("=" * 80)
    print(f"Train seasons: {oos['train_seasons']}")
    ...
    for phase in ["pp", "mid", "death"]:
        if phase in oos["phases"]:
            ...
            rows.append({
                "phase": phase,
                "oos_brier_raw": round(p["brier_raw"], 5),
                "oos_brier_cal": round(p["brier_cal"], 5),
                "n": p["n"],
            })
    ...
    if rows:
        print("-" * 50)
        print(f"{'OVERALL':<10} {oos['overall_raw']:>10.5f} {oos['overall_cal']:>10.5f} {sum(r['n'] for r in rows):>8,}")
        rows.append({
            "phase": "overall",
            "oos_brier_raw": round(oos["overall_raw"], 5),
            "oos_brier_cal": round(oos["overall_cal"], 5),
            "n": sum(r["n"] for r in rows[:-1]) if len(rows) > 1 else rows[0]["n"],
        })
    ...
    print(f"\n{'=' * 60}")
    print("NTB v1 PHASE BUILD COMPLETE")
    print("=" * 60)
