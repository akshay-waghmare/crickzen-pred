...
def _source_checks(
    ...
    *,
    ...
    checks = [
        SourceCheck(
            name="fixture",
            passed=verify_expected_match(snapshot, expected_match),
            detail="Fixture verified." if verify_expected_match(snapshot, expected_match) else "Fixture does not match the requested teams/date.",
        ),
        SourceCheck(
            name="freshness",
            passed=is_fresh(snapshot.source_timestamp, now=now, max_age_minutes=max_age_minutes),
            detail="Model state is fresh." if is_fresh(snapshot.source_timestamp, now=now, max_age_minutes=max_age_minutes) else "Model state is stale or missing a timestamp.",
        ),
